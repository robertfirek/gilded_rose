...
@pytest.mark.parametrize("sample_number", [sample_number for sample_number in range(1000)])
def test_gilded_rose_should_update_items(sample_number):
    items_file = open(f"tests/unit/golden_master_samples/input_{sample_number:04d}.pkl", "rb")
    items = pickle.load(items_file)
    items_after_update_file = open(
        f"tests/unit/golden_master_samples/output_{sample_number:04d}.pkl", "rb")
    ...
    print(items)
    print("\nExpected output:")
    print(expected_items_after_update)
